...
int CSetOfSeq::CheckSeq(const char* file)
{
   ...
   while(!feof(f))
   {
      	...
      	if (strlen(line) > 2)
      	{
          	...
          	if (strcmp(line, "##########")==0)
          	{
            	m_vSeqLength.push_back(counter);
              	counter = 0;
          	}
          	else
          	{
      			int iColumn = count_column(line, '\t');
      			//printf("line:%d, seq: %d, column %d\n", linecount, counter, iColumn);
...
int CSetOfSeq::LoadSS(const char* strFile)
{
    ...
    for (int i = 0; i < (int)m_vSeqLength.size(); i ++)
    {
        ...
        if (strcmp(line, "##########") != 0)
        {
    	    fprintf(stderr, "can't find the seq separator ########## at file %s , found |%s| instead \n", strFile, line);
            exit(1);
        }
...
int CModel::Read(const char* strFile)
{
    ...
    for (int i = 0; i < itotaline - 3; i ++)
    {
    	...
    	//fprintf(stderr, "%f %d %d %d %d %d", w, idx, wid, maptype, matchgap, sign);
